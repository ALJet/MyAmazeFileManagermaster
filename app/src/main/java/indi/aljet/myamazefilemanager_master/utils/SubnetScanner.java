...
public class SubnetScanner extends Thread {
    ...
    class Task implements Callable<ComputerParcelable>{
        ...
        public ComputerParcelable call() throws Exception {
            try{
                NbtAddress[] allByAddress = NbtAddress.getAllByAddress
                        (this.addr);
                if(allByAddress == null || allByAddress
                        .length <= 0){
                    return new ComputerParcelable(null,
                            this.addr);
                }
                return new ComputerParcelable(allByAddress[0]
                .getHostName(),this.addr);
    ...
    private void tryWithBroadcast(){
        this.bdThread = new Thread(){
            @Override
            public void run() {
                for (int i = 0;i < SubnetScanner
                        .RETRY_COUNT;i++){
                    try {
                        SmbFile smbFile = new SmbFile("smb://");
                        smbFile.setConnectTimeout(5000);
                        SmbFile[] listFiles = smbFile
                                .listFiles();
                        for(SmbFile smbFile2 : listFiles){
                            SmbFile[] listFiles2 = smbFile2
                                    .listFiles();
                            for(SmbFile files : listFiles2){
                                try{
                                    String substring =
                                            ...
                                            .getName().length() - 1);
                                    UniAddress byName =
                                            UniAddress.getByName(substring);
                                    if(byName != null){
                                        SubnetScanner.this
                                                .onFound(new ComputerParcelable
