...
/**
 * Created by PC-LJL on 2018/1/29.
 */
...
public class GenericCopyUtil {
    ...
    public static final String PATH_FILE_DESCRIPTOR =
            "/proc/self/fd/";

    public static final int DEFAULT_BUFFER_SIZE = 8192;

    public GenericCopyUtil(Context mContext) {
        this.mContext = mContext;
    }


    private void startCopy(boolean lowOnMemory)
        throws IOException{
    ...
    public void copy(HybridFileParcelable sourceFile,
    ...
    private void copyFile(BufferedInputStream bufferedInputStream,
                          FileChannel outChannel)
    throws IOException{
        MappedByteBuffer byteBuffer = outChannel
                .map(FileChannel
    ...
    private void copyFile(FileChannel inChannel,
                          FileChannel outChannel)
        throws IOException{
        ReadableByteChannel inByteChannel = new
                CustomReadableByteChannel(inChannel);
        outChannel.transferFrom(inByteChannel,0,
                mSourceFile.getSize());
    }
    ...
    throws IOException{
        MappedByteBuffer inBuffer = inChannel.map
                (FileChannel.MapMode.READ_ONLY,0,mSourceFile
                .getSize());
        int count = -1;
        byte[] buffer = new byte[DEFAULT_BUFFER_SIZE];
        while(inBuffer.hasRemaining() && count != 0){
            ...
            try{
                ByteBuffer tempByteBuffer = inBuffer
                        .get(buffer);
                count = tempByteBuffer.position() -
                        tempPosition;
            }catch (BufferUnderflowException exception){
                exception.printStackTrace();
                count = 0;
                for(int i = 0;i < buffer.length&& inBuffer
                        .hasRemaining();i++){
            ...
            if(count != -1){
                ...
                ServiceWatcherUtil.POSITION = inBuffer
                        .position();
            }
        }
        bufferedOutputStream.flush();
    ...
    private class CustomReadableByteChannel implements
            ReadableByteChannel{
        ReadableByteChannel byteChannel;

        public CustomReadableByteChannel(ReadableByteChannel
                                         byteChannel) {
