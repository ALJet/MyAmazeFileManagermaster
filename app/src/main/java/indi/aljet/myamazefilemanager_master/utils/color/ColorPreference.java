...
public class ColorPreference {
    ...
    public static final List<Integer> availableColors =
            Arrays.asList(
                    R.color.primary_red,
    ...
    public static ColorPreference loadFromPreferences
            (Context context,
        ...
        for(ColorUsage usage : ColorUsage.values()){
            try{
                int val = preferences.getInt(usage
                .asString(),colorIndex(usage.getDefaultColor()));
    ...
    public ColorPreference randomize(){
                int[] colorPos = combinations[new Random()
                        .nextInt(combinations.length - 1)];
                setRes(ColorUsage.PRIMARY,getColorByIndex(colorPos
                [0]));
                setRes(ColorUsage.PRIMARY_TWO,
                        getColorByIndex(colorPos[0]));
                setRes(ColorUsage.ACCENT,
                        getColorByIndex(colorPos[1]));
                setRes(ColorUsage.ICON_SKIN,
                        getColorByIndex(colorPos[2]));
                return this;
            }


    public ColorPreference saveToPreferences
            (SharedPreferences preferences){
        SharedPreferences.Editor e = preferences.edit();
        for(ColorUsage usage : colors.keySet()){
            e.putInt(usage.asString(),colorIndex(
                    getRes(usage)));
        }
        e.apply();
    ...
    public ColorPreference setRes(ColorUsage usage,
                                  @ColorRes int color){
