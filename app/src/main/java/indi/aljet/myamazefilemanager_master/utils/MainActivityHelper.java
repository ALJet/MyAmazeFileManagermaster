...
import indi.aljet.myamazefilemanager_master.asynchronous.services.ZipService;
...
import indi.aljet.myamazefilemanager_master.fragments.TabFragment;
...
public class MainActivityHelper {
    ...
    public MainActivityHelper(MainActivity mainActivity) {
        ...
        accentColor = mainActivity.getColorPreference()
                .getColor(ColorUsage.ACCENT);
    }

    public void showFailedOperationDialog(
            ArrayList<HybridFileParcelable> failedOps,
            boolean move, Context context){
        MaterialDialog.Builder mat = new MaterialDialog.
                Builder(context);
        mat.title(context.getString(R.string
        .operationunsuccesful));
        mat.theme(mainActivity.getAppTheme().
        getMaterialDialogTheme());
        mat.positiveColor(accentColor);
        mat.positiveText(R.string.cancel);
        String content = context.getResources()
                .getString(R.string.operation_fail_following);
        int k = 1;
    ...
    public final BroadcastReceiver mNotificationReceiver = new
            BroadcastReceiver() {
                @Override
                    ...
                    if (intent != null) {
                        if (intent.getAction().equals(Intent
                            ...
                            Toast.makeText(mainActivity,
                                    "Media Mounted", Toast
                                            .LENGTH_SHORT).show();
                            String a = intent.getData()
                                    .getPath();
                            if (a != null && a.trim().length() != 0
                                    && new File(a).exists() && new
                                    File(a).canExecute()) {
                                dataUtils.getStorages()
                                        .add(a);
            ...
            void mkdir(final OpenMode openMode,final String path,
                ...
                mk(R.string.newfolder,materialDialog ->{
                    String a = materialDialog.getInputEditText()
                            .getText().toString();
                    mkDir(new HybridFile(openMode,path
                    + "/" + a),ma);
    ...
    void mkfile(final OpenMode openMode,final String path,
                ...
                mk(R.string.newfile,materialDialog -> {
                    String a = materialDialog.getInputEditText()
                            .getText().toString();
                    mkFile(new HybridFile(openMode,path + "/"
                    + a ),ma);
                    materialDialog.dismiss();
                });
    }
    ...
    private void mk(@StringRes int newText,
                    final OnClickMaterialListener l){
        final MaterialDialog materialDialog =
                GeneralDialogCreation.showNameDialog
                        (mainActivity,new String[] {mainActivity
                        ...
                        null});
        materialDialog.getActionButton(DialogAction
        .POSITIVE).setOnClickListener( v -> l.onClick(
                materialDialog
        ));
        materialDialog.show();
    }
    ...
    public void add(int pos){
        final MainFragment ma = (MainFragment)
                ((TabFragment)mainActivity.getSupportFragmentManager()
        ...
        switch (pos){
            ...
            case NEW_FILE:
                ...
                break;
            case NEW_CLOUD:
                BottomSheetDialogFragment fragment = new
                        CloudSheetFragment();
                fragment.show(ma.getActivity()
    ...
    public String getIntegralNames(String path){
        ...
        switch (Integer.parseInt(path)){
            case 0:
                newPath = mainActivity.getResources()
    ...
    public void guideDialogForLEXA(String path){
        final MaterialDialog.Builder x = new
                MaterialDialog.Builder(mainActivity);
        x.theme(mainActivity.getAppTheme()
        .getMaterialDialogTheme());
        x.title(R.string.needsaccess);
        LayoutInflater layoutInflater = (LayoutInflater)
                mainActivity.getSystemService(Context
        ...
        .id.description);
        textView.setText(mainActivity.getResources()
        .getString(R.string.needsaccesssummary) + path +
        ...
        x.callback(new MaterialDialog.ButtonCallback() {
            ...
            public void onNegative(MaterialDialog dialog) {
                Toast.makeText(mainActivity,R.string
    ...
    public void rename(OpenMode mode, final String oldPath,
        ...
        final Toast toast = Toast.makeText(context,
                ...
                Toast.LENGTH_SHORT);
        toast.show();
        Operations.rename(new HybridFile(mode,oldPath),
                new HybridFile(mode,newPath),rootmode
    ...
    public static final int DOESNT_EXIT = 0;
    ...
    public int checkFolder(final File folder,Context context){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
            ...
            if(FileUtil.isOnExtSdCard(folder,context)){
                return WRITABLE_OR_ON_SDCARD;
            }else if(FileUtil.isWritable(new File(folder,
                    "DummyFile"))){
                return WRITABLE_OR_ON_SDCARD;
            }else
                return DOESNT_EXIT;
    ...
    public void compressFiles(File file,
        ...
        int mode = checkFolder(file.getParentFile(),
                mainActivity);
        if(mode == 2){
            mainActivity.oppathe = (file.getPath());
            ...
            Intent intent2 = new Intent(mainActivity,
                    ZipService.class);
            intent2.putExtra(ZipService.KEY_COMPRESS_PATH,
                    file.getPath());
            intent2.putExtra(ZipService.KEY_COMPRESS_FILES,
                    baseFiles);
            ServiceWatcherUtil.runService(mainActivity,
                    intent2);

        }else {
            Toast.makeText(mainActivity,R.string
            .not_allowed,Toast.LENGTH_SHORT).show();
        }
    }


    public void mkFile(final HybridFile path,
                       final MainFragment ma){
        final Toast toast = Toast.makeText(ma
        .getActivity(),ma.getString(R.string.creatingfile),
                Toast.LENGTH_SHORT);
        toast.show();
        Operations.mkfile(path,ma.getActivity(),
                ThemedActivity.rootMode,new Operations
        .ErrorCallBack(){
                    @Override
    ...
    public void mkDir(final HybridFile path,
                      final MainFragment ma){
        final Toast toast = Toast.makeText(ma.getActivity()
        ,ma.getString(R.string.creatingfolder),
                Toast.LENGTH_SHORT);
        toast.show();
    ...
    public void deleteFiles(ArrayList<HybridFileParcelable>
                            files){
        if(files == null || files.size() == 0){
            return;
        }
        if(files.get(0).isSmb()){
            new DeleteTask(null,mainActivity)
                    .execute((files));
            return;
        }
        int mode = checkFolder(new File(files.get(0)
        .getPath()).getParentFile(),mainActivity);
        if(mode == 2){
    ...
    public void extractFile(File file) {
        ...
        if (mode == 2) {
            ...
            CompressedInterface compressedInterface =
                    CompressedHelper.getCompressedInterfaceInstance
                            (mainActivity, file);
    ...
    public String parseSmbPath(String a){
        if(a.contains("@")){
            return "smb://" + a.substring(a.indexOf("@")
                    + 1,a.length());
        }else{
            return a;
        }
    ...
    public String parseOTGPath(String path){
        if(path.contains(OTGUtil.PREFIX_OTG)){
            ...
            return OTGUtil.PREFIX_OTG + path.substring(
                    path.indexOf(":") + 1,path.length());
        }
    }


    public String parseCloudPath(OpenMode serviceType,
                                 String path){
        switch (serviceType){
            case DROPBOX:
                if(path.contains(CloudHandler
                .CLOUD_PREFIX_DROPBOX)){
                    return path;
                }else{
                    return CloudHandler.CLOUD_PREFIX_DROPBOX
                            + path.substring(path
                    .indexOf(":") + 1,path.length());
                }
            case BOX:
                if(path.contains(CloudHandler
                .CLOUD_NAME_BOX))
            ...
            case ONEDRIVE:
                if(path.contains(CloudHandler.CLOUD_PREFIX_ONE_DRIVE))
                    ...
                    return CloudHandler.CLOUD_NAME_ONE_DRIVE + path
    ...
    public void search(SharedPreferences sharedPreferences,
                       String query){
        TabFragment tabFragment = mainActivity
                .getTabFragment();
        if(tabFragment == null){
            return;
        }
        ...
        final MainFragment ma = (MainFragment)
                tabFragment.getCurrentTabFragment();
        final String fpath = ma.getCurrentPath();
        SEARCH_TEXT = query;
        mainActivity.mainFragment = (MainFragment)
                mainActivity.getTabFragment()
                .getCurrentTabFragment();
        FragmentManager fm = mainActivity
                .getSupportFragmentManager();
        SearchWorkerFragment fragment =
                (SearchWorkerFragment)fm
                .findFragmentByTag(MainActivity
                .TAG_ASYNC_HELPER);

        if(fragment != null){
            if(fragment.mSearchAsyncTask.getStatus() ==
        ...
        addSearchFragment(fm,new SearchWorkerFragment(),
                fpath,query,ma.openMode,
                ThemedActivity.rootMode,
                sharedPreferences.getBoolean(SearchWorkerFragment
                .KEY_REGEX,false),
                sharedPreferences.getBoolean(SearchWorkerFragment
    ...
    public static void addSearchFragment(FragmentManager
                                         fragmentManager,
                                         Fragment fragment,
                                         String path,
                                         String input,
        ...
        args.putString(SearchWorkerFragment.KEY_INPUT,
                input);
