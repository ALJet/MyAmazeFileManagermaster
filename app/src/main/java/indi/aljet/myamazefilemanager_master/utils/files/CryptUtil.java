...
import indi.aljet.myamazefilemanager_master.utils.ProgressHandler;
...
public class CryptUtil {
    ...
    public CryptUtil(Context context,
        ...
        HybridFile hFile = new HybridFile(sourceFile
                .getMode(), sourceFile.getParent(context));
    ...
    public CryptUtil(Context context,
        ...
        this.progressHandler = progressHandler;
        this.failedOps = failedOps;
        HybridFile targetdirectory = new
                HybridFile(OpenMode.FILE,
                targetpath);
        if (!targetdirectory.equals(context.getExternalCacheDir())) {
    ...
    private static String aesEncryptPassword
        ...
        GCMParameterSpec gcmParameterSpec = new
                GCMParameterSpec(128, IV.getBytes());
        byte[] encodedBytes = cipher.doFinal(
                plainTextPassword.getBytes());
        return Base64.encodeToString(encodedBytes,
                Base64.DEFAULT);
    }


    @RequiresApi(api = Build.VERSION_CODES.M)
    private static String aesDecryptPassword(String
                                                     cipherPassword)
            throws GeneralSecurityException, IOException {
        Cipher cipher = Cipher.getInstance(ALGO_AES);
        GCMParameterSpec gcmParameterSpec = new
        ...
        byte[] decryptedBytes = cipher
                .doFinal(Base64.
                        decode(cipherPassword, Base64.DEFAULT));
        return new String(decryptedBytes);
    }

    @RequiresApi(api = Build.VERSION_CODES.M)
    private static void aesEncrypt(BufferedInputStream
                                           inputStream, BufferedOutputStream outputStream)
            throws GeneralSecurityException, IOException {
        Cipher cipher = Cipher.getInstance(ALGO_AES);
    ...
    @RequiresApi(api = Build.VERSION_CODES.M)
    private static void aesDecrypt(BufferedInputStream
                                           inputStream,
        ...
        byte[] buffer = new byte[GenericCopyUtil
                .DEFAULT_BUFFER_SIZE];
        int count;
    ...
    private static Key getSecretKey()
        ...
        if(!keyStore.containsAlias(KEY_ALIAS_AMAZE)){
            ...
            KeyGenParameterSpec.Builder builder =
                    new KeyGenParameterSpec.Builder(KEY_ALIAS_AMAZE,
                            KeyProperties.PURPOSE_ENCRYPT |
                    KeyProperties.PURPOSE_DECRYPT);
            builder.setBlockModes(KeyProperties
            .BLOCK_MODE_GCM);
            builder.setEncryptionPaddings(KeyProperties
