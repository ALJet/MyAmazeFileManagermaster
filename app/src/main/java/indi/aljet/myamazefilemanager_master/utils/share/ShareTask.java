...
public class ShareTask extends AsyncTask<String,String,Void> {
    ...
    private ArrayList<Intent> targetShareIntents = new
            ArrayList<>();

    private ArrayList<String> arrayList1 =
            new ArrayList<>();
    private ArrayList<Drawable> arrayList2 =
            new ArrayList<>();
    ...
    protected Void doInBackground(String... strings) {
        ...
        PackageManager packageManager = contextc
                .getPackageManager();
        ...
        if(!resInfos.isEmpty()){
            for(ResolveInfo resInfo : resInfos){
                ...
                arrayList1.add(resInfo.loadLabel(
                        packageManager).toString());
                if(packageName.contains("android.bluetooth")){
                    bluetooth_present = true;
                }
                Intent intent = new Intent();
                System.out.println(resInfo.activityInfo
                        .packageName + "\t" +
                ...
                intent.setType(mime);
                intent.putParcelableArrayListExtra(Intent
                .EXTRA_STREAM,arrayList);
                intent.setPackage(packageName);
                targetShareIntents.add(intent);

            }
        }
        if(!bluetooth_present && appInstalledOrNot("com.android.bluetooth",
                packageManager)){
            Intent intent = new Intent();
            ...
            targetShareIntents.add(intent);
            arrayList1.add(contextc.getResources()
            .getString(R.string.bluetooth));
            arrayList2.add(contextc.getResources()
            .getDrawable(appTheme.equals(AppTheme
            .LIGHT) ? R.drawable.ic_settings_bluetooth_black_24dp :
    ...
    protected void onPostExecute(Void aVoid) {
        if(!targetShareIntents.isEmpty()){
            MaterialDialog.Builder builder = new
                    MaterialDialog.Builder(contextc);
            builder.title(R.string.share);
            builder.theme(appTheme
            .getMaterialDialogTheme());
            ShareAdapter shareAdapter = new ShareAdapter
                    (contextc, targetShareIntents,
                            arrayList1,arrayList2);
            builder.adapter(shareAdapter,null);
            builder.negativeText(R.string.cancel);
            builder.negativeColor(fab_skin);
            MaterialDialog b = builder
                    .build();
            shareAdapter.updateMatDialog(b);
            b.show();
        }else{
            Toast.makeText(contextc, R.string.noappfound,
