...
import indi.aljet.myamazefilemanager_master.ui.views.preference.PathSwitchPreference;
import indi.aljet.myamazefilemanager_master.utils.DataUtils;
import indi.aljet.myamazefilemanager_master.utils.SimpleTextWatcher;
...
implements Preference.OnPreferenceClickListener{
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        findPreference(KEY_SHORTCUT_PREF).
        setOnPreferenceClickListener(this);

        for(int i = 0;i < dataUtils.getBooks().size();i++){
            PathSwitchPreference p = new
                    PathSwitchPreference(getActivity());
    ...
    private void loadCreateDialog(){
        int fab_skin = activity.getColorPreference()
    ...
    private void disableButtonIfNotPath(EditText path,
                                        final MaterialDialog dialog){
        path.addTextChangedListener(new SimpleTextWatcher(){
            @Override
            public void afterTextChanged(Editable editable) {
                dialog.getActionButton(DialogAction.POSITIVE)
                        .setEnabled(FileUtils.isPathAccesible
                                (editable.toString(),sharedPrefs));
            }
        });
    }
