...
import indi.aljet.myamazefilemanager_master.utils.CopyDataParcelable;
...
/**
 * Created by PC-LJL on 2018/2/11.
 */
    ...
    private ServiceConnection mConnection = new
            ServiceConnection() {
                ...
                public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
                    CopyService.LocalBinder localBinder = (CopyService
                    .LocalBinder)iBinder;
                    CopyService copyService = localBinder
                            .getService();
                    for(int i = 0;i < copyService.getDataPackageSize();
                            i++){
                        processResults(copyService.getDataPackage(i),
                                ServiceType.COPY);
                    }
                    mLineChart.animateXY(500,500);
                    copyService.setProgressListener(new CopyService.ProgressListener() {
                        @Override
                        public void onUpdate(CopyDataParcelable dataPackage) {
