...
import indi.aljet.myamazefilemanager_master.asynchronous.asynctasks.LoadFilesListTask;
import indi.aljet.myamazefilemanager_master.asynchronous.handlers.FileHandler;
...
import indi.aljet.myamazefilemanager_master.filesystem.MediaStoreHack;
...
import indi.aljet.myamazefilemanager_master.utils.SmbStreamer.Streamer;
...
public class MainFragment extends Fragment
     ...
     * @param imageView the check {@link } that is to be animated
     */
     ...
     * @return the entry
     */
    private static Encrypted findEncryptedEntry(Context context, String path) throws Exception {

        CryptHandler handler = new CryptHandler(context);

        Encrypted encrypt = null;
        // find closest path which matches with database entry
        for (Encrypted encrypted : DBHelper.getAllEntrpteds()) {
            if (path.contains(encrypted.getPath())) {

                if (encrypted == null || encrypted.getPath().length() < encrypted.getPath().length()) {
                    encrypt = encrypted;
                }
            }
        }
    ...
    public boolean checkPathIsGrid(String path){
        ...
        if(!both_contain)
            return grid;
        String path1 = dataUtils.getGridFiles().get(i1);
        String path2 = dataUtils.getListfiles().get(2);
