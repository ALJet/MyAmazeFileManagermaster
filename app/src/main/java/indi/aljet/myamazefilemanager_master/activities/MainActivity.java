...
import indi.aljet.myamazefilemanager_master.adapter.DrawerAdapter;
...
import indi.aljet.myamazefilemanager_master.asynchronous.asynctasks.MoveFiles;
import indi.aljet.myamazefilemanager_master.asynchronous.asynctasks.PrepareCopyTask;
...
import indi.aljet.myamazefilemanager_master.database.models.CloudEntry;
...
import indi.aljet.myamazefilemanager_master.fragments.FTPServerFragment;
import indi.aljet.myamazefilemanager_master.fragments.MainFragment;
import indi.aljet.myamazefilemanager_master.fragments.ProcessViewerFragment;
...
import indi.aljet.myamazefilemanager_master.ui.dialogs.RenameBookmark;
import indi.aljet.myamazefilemanager_master.ui.dialogs.SmbConnectDialog;
import indi.aljet.myamazefilemanager_master.ui.views.ScrimInsetsRelativeLayout;
import indi.aljet.myamazefilemanager_master.ui.views.appbar.AppBar;
import indi.aljet.myamazefilemanager_master.utils.BookSorter;
...
import static indi.aljet.myamazefilemanager_master.fragments.preference_fragments.PrefFrag.PREFERENCE_SHOW_SIDEBAR_FOLDERS;
import static indi.aljet.myamazefilemanager_master.fragments.preference_fragments.PrefFrag.PREFERENCE_SHOW_SIDEBAR_QUICKACCESSES;

public class MainActivity extends ThemedActivity implements
        ActivityCompat.OnRequestPermissionsResultCallback,
        SmbConnectDialog.SmbConnectionListener,
        DataUtils.DataChangeListener,
        RenameBookmark.BookmarkCallback,
        SearchWorkerFragment.HelperCallbacks,
        CloudSheetFragment.CloudConnectionCallbacks,
   ...
   */
    ...
    private static final String CLOUD_AUTHENTICATOR_REDIRECT_URI = "indi.aljet.myamazefilemanager_master.filemanager:/oauth2redirect";
    ...
    protected void onPause() {
        super.onPause();
        unregisterReceiver(mainActivityHelper
    ...
    private void closeInteractiveShell() {
        if (rootMode) {
            if (SDK_INT >= Build.VERSION_CODES
                    .JELLY_BEAN_MR2) {
    ...
    protected void onActivityResult(int requestCode, int responseCode, Intent intent) {
        if (requestCode == image_selector_request_code) {
            ...
            if (responseCode == Activity.RESULT_OK) {
                ...
                if (treeUri != null)
                    getPrefs().edit().putString("URI", treeUri.toString()).commit();
            } else {
    ...
    void initialisePreferences() {
        ...
        currentTab = getPrefs().getInt(PreferenceUtils.
                KEY_CURRENT_TAB, PreferenceUtils.
                DEFAULT_CURRENT_TAB);
        skinStatusBar = (PreferenceUtils.getStatusColor(getColorPreference().
    ...
    void initialiseViews() {
        ...
        if (getAppTheme().equals(AppTheme.DARK))
            ...
            mDrawerLinear.setBackgroundColor(Utils.getColor(this, android.R.color.black));
    ...
    public void onNewIntent(Intent i) {
        ...
        if (path != null) {
            ...
            CloudRail
                    .setAuthenticationResponse(intent);
...
//                            ((MainFragment) main1).updateTabWithDb(tabHandler.findTab(2));
    ...
    @Override
    public void addConnection(boolean edit, String name, String path, String encryptedPath, String oldname, String oldPath) {
        ...
        if (!edit) {
            if ((dataUtils.containsServer(path)) == -1) {
                ...
                refreshDrawer();
                AppConfig.runInBackground(() -> {
                    utilsHandler.addSmb(name,
                            encryptedPath);
                });
                MainFragment ma = getCurrentMainFragment();
                if (ma != null) {
                    getCurrentMainFragment()
                            .loadlist(path, false, OpenMode.UNKNOWN);
                }

            } else {
                Snackbar.make(frameLayout,
                        getResources().getString(R.
                                string.connection_exists),
                        Snackbar.LENGTH_SHORT)
                        .show();
            }
        } else {
            int i = dataUtils
                    .containsServer(new String[]{
                            oldname, oldPath});
            if (i != -1) {
    ...
    public void deleteConnection(final String name,
                                 final String path) {
        int i = dataUtils.containsServer
    ...
    @Override
    public void onHiddenAdded(String path) {
        utilsHandler.addHidden(path);
    }

    @Override
    public void onBookAdded(String[] path, boolean refresahdrawer) {
        utilsHandler.addBookmark(path[0],
                path[1]);
        if(refresahdrawer)
            refreshDrawer();
    }
    ...
    public void delete(String title, String path) {
        utilsHandler.removeBookmarksPath(title,
                path);
        refreshDrawer();
    }
    ...
    public void modify(String oldpath, String oldname, String newpath, String newname) {
        utilsHandler.renameBookmark(oldname,
                oldpath,
                newname,newPath);
        refreshDrawer();
    }
    ...
    public void addConnection(OpenMode service) {
        if (DBHelper.findEntrypted(service) != null) {
    ...
    public Loader<Cursor> onCreateLoader(int id,
                                         Bundle args) {
        if(cloudSyncTask != null && cloudSyncTask
                .getStatus() == AsyncTask
                .Status.RUNNING){
            cloudSyncTask.cancel(true);
        }
        Uri uri = Uri.withAppendedPath(Uri.
                parse("content://" + CloudContract.
                        PROVIDER_AUTHORITY),
                "/keys.db/secret_keys");
        ...
        switch (id) {
            ...
            case REQUEST_CODE_CLOUD_LIST_KEYS:
                ...
                try {
//                    List<Encrypted> cloudEntries = DBHelper.getAllEntrpteds();
                    List<CloudEntry> cloudEntries = cloudHandler.getAllEntries();

//                    List<Cloud> clouds = DBHelper.getAllEntries();
