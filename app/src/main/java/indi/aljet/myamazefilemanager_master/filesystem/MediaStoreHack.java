...
public class MediaStoreHack {
    ...
    private static File getExternalFilesDir(final
                                            Context
                                            context){
    ...
    public static InputStream getInputStream(final Context context,
                                             ...
                                             final long size){
        try{
            final String where = MediaStore.MediaColumns
                    .DATA + " = ?";
            final String[] selectionArgs = new String[] {
                    file.getAbsolutePath()
            };
            final ContentResolver contentResolver = context
                    .getContentResolver();
            final Uri filesUri = MediaStore
                    .Files.getContentUri("external");
            contentResolver.delete(filesUri,where,
                    selectionArgs);
            final ContentValues values = new ContentValues();
    ...
    public static Uri getUriFromFile(final String path,
        ...
        if(filecursor.isAfterLast()){
            ...
            .getColumnIndex(BaseColumns
            ._ID));
            Uri uri = MediaStore.Files.getContentUri
                    ("external").buildUpon()
