...
public class PrepareCopyTask extends AsyncTask
<ArrayList<HybridFileParcelable>,String ,PrepareCopyTask.CopyNode>{
    ...
    protected CopyNode doInBackground(ArrayList<HybridFileParcelable>[] params) {
        ...
        HybridFile destination = new HybridFile(openMode,
                path);
    ...
    @Override
    protected void onPostExecute(CopyNode copyNode) {
        super.onPostExecute(copyNode);
        if (openMode == OpenMode.OTG
                || openMode == OpenMode.GDRIVE
                || openMode == OpenMode.DROPBOX
                || openMode == OpenMode.BOX
    ...
    class CopyNode{
        ...
        private ArrayList<HybridFileParcelable> filesToCopy,
                conflictingFiles;
        private ArrayList<CopyNode> nextNodes = new
                ArrayList<>();

        CopyNode(String p,ArrayList<HybridFileParcelable> fileToCopy){
            path = p;
