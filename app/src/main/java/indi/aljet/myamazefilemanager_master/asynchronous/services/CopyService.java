package indi.aljet.myamazefilemanager_master.asynchronous.services;
            ...
            if (sourceFile.isDirectory() && !sourceFile.getName().endsWith(CryptUtil.CRYPT_EXTENSION)) {
                ...
                if (sourceFile.getName().endsWith(CryptUtil.CRYPT_EXTENSION)) {
                    try {
                        ...
                        Encrypted encrypted = DBHelper.findEntrypted(sourceFile.getPath());
...
//                        newEntry.setPath(targetPath + "/" + sourceFile.getName());
     ...
     * data to be transferred to {@link }
     ...
     * is executing the callbacks in {@link }
     *
     * @return
     */
