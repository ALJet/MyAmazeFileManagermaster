...
import static indi.aljet.myamazefilemanager_master.utils.Utils.getColor;

/**
 * Created by PC-LJL on 2018/1/31.
 */

public class LoadFolderSpaceDataTask extends AsyncTask
<Void,Long,Pair<String,List<PieEntry>>>{
    ...
    @Override
    protected Pair<String, List<PieEntry>> doInBackground(Void... voids) {
        long[] dataArray = FileUtils.getSpaces(file,
                context,this:: publishProgress);
        if(dataArray[0] != -1 && dataArray[0] != 0){
            long totalSpace = dataArray[0];
            List<PieEntry> entries = createEntriesFromArray(dataArray,
                    false);
            return new Pair<>(Formatter.formatFileSize(context,
                    totalSpace),entries);
        }
        return null;
    }
    ...
    protected void onProgressUpdate(Long... dataArray) {
        if(dataArray[0] != -1 && dataArray[0] != 0){
            long totalSpace = dataArray[0];
            List<PieEntry> entries = createEntriesFromArray(new
            long[] {dataArray[0], dataArray[1], dataArray[2]},
                    true);
            updateChart(Formatter.formatFileSize(context,
                    totalSpace),entries);
    ...
    private List<PieEntry> createEntriesFromArray(long[] dataArray,
                                                  boolean loading){
    ...
    private void updateChart(String totalSpace,
